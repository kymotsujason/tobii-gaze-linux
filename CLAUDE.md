...
- Gameplay monitor is **DP-2**, an Alienware AW2725DF, and it is the X11 primary.
  ...
  all three were wrong, but not invented. **`DP-1-2` is a real output**: there are eight
  connected across two GPUs, and DP-1-2 is a 597 x 336 mm panel at exactly +4000+0, sitting
  directly ABOVE the gameplay monitor. So the plan's output name, offset and size were
  mutually consistent and simply described the WRONG SCREEN, which is a far easier mistake
  to make than a wrong number and much harder to notice. Always confirm which output the
  tracker is physically attached to. EDID reports this panel as 590 x 330 because EDID
  stores whole centimetres and 333.72 truncates to 33, so trust the spec sheet over EDID.
